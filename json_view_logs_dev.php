...
$qry_log = mysql_query("SELECT DISTINCT date from tbl_logs WHERE company_id = '$company_id' AND emp_id = '$emp_id' ORDER BY date DESC");
while($row_log = mysql_fetch_array($qry_log)){
	...
	$row_fn = mysql_fetch_array($qry_fn);
		 ...
		 $sl = $row_fn['sick_leave'];
		 $vl = $row_fn['vacation_leave'];
	$qry_timein = mysql_query("SELECT * FROM tbl_logs WHERE company_id = '$company_id' AND date = '$date' AND emp_id = '$emp_id' AND status = 'Attendance' AND action = 'In' ORDER BY log_id ASC");
	$row_timein = mysql_fetch_array($qry_timein);
		$attendance_in = $row_timein['time'];
		if($attendance_in == ""){
			$attendance_in = "-";
		}
	$qry_lunchout = mysql_query("SELECT * FROM tbl_logs WHERE company_id = '$company_id' AND date = '$date' AND emp_id = '$emp_id' AND status = 'Lunch' AND action = 'Out' ORDER BY log_id ASC");
	$row_lunchout = mysql_fetch_array($qry_lunchout);
		$lunch_out = $row_lunchout['time'];
		if($lunch_out == ""){
			$lunch_out = "-";
		}
	$qry_lunchin = mysql_query("SELECT * FROM tbl_logs WHERE company_id = '$company_id' AND date = '$date' AND emp_id = '$emp_id' AND status = 'Lunch' AND action = 'In' ORDER BY log_id DESC");
	$row_lunchin = mysql_fetch_array($qry_lunchin);
		$lunch_in = $row_lunchin['time'];
		if($lunch_in == ""){
			$lunch_in = "-";
		}
	$qry_meetingout = mysql_query("SELECT * FROM tbl_logs WHERE company_id = '$company_id' AND date = '$date' AND emp_id = '$emp_id' AND status = 'Meeting' AND action = 'Out' ORDER BY log_id ASC");
	$row_meetingout = mysql_fetch_array($qry_meetingout);
		$meeting_out = $row_meetingout['time']." - ".$row_meetingout['company'];
		if($meeting_out == ""){
			$meeting_out = "-";
		}
	$qry_meetingin = mysql_query("SELECT * FROM tbl_logs WHERE company_id = '$company_id' AND date = '$date' AND emp_id = '$emp_id' AND status = 'Meeting' AND action = 'In' ORDER BY log_id DESC");
	$row_meetingin = mysql_fetch_array($qry_meetingout);
		$meeting_in = $row_meetingin['time'];
		if($meeting_in == ""){
			$meeting_in = "-";
		}
	$qry_timeout = mysql_query("SELECT * FROM tbl_logs WHERE company_id = '$company_id' AND date = '$date' AND emp_id = '$emp_id' ORDER BY log_id DESC");
...
$qry_fn = mysql_query("SELECT * FROM tbl_employee WHERE employee_id = '$emp_id'");
	$row_fn = mysql_fetch_array($qry_fn);
		 ...
		 $sl = $row_fn['sick_leave'];
		 $vl = $row_fn['vacation_leave'];
$qry_log = mysql_query("SELECT DISTINCT date from tbl_logs WHERE emp_id = '$emp_id' ORDER BY date DESC");
while($row_log = mysql_fetch_array($qry_log)){
		$date = $row_log['date'];
	$qry_fn = mysql_query("SELECT * FROM tbl_employee WHERE employee_id = '$emp_id'");
	$row_fn = mysql_fetch_array($qry_fn);
		 ...
		 $sl = $row_fn['sick_leave'];
		 $vl = $row_fn['vacation_leave'];
	$qry_timein = mysql_query("SELECT * FROM tbl_logs WHERE date = '$date' AND emp_id = '$emp_id' AND status = 'Attendance' AND action = 'In' ORDER BY log_id ASC");
	$row_timein = mysql_fetch_array($qry_timein);
		$attendance_in = $row_timein['time'];
		if($attendance_in == ""){
			$attendance_in = "-";
		}
	$qry_lunchout = mysql_query("SELECT * FROM tbl_logs WHERE date = '$date' AND emp_id = '$emp_id' AND status = 'Lunch' AND action = 'Out' ORDER BY log_id ASC");
	$row_lunchout = mysql_fetch_array($qry_lunchout);
		$lunch_out = $row_lunchout['time'];
		if($lunch_out == ""){
			$lunch_out = "-";
		}
	$qry_lunchin = mysql_query("SELECT * FROM tbl_logs WHERE date = '$date' AND emp_id = '$emp_id' AND status = 'Lunch' AND action = 'In' ORDER BY log_id DESC");
	$row_lunchin = mysql_fetch_array($qry_lunchin);
		$lunch_in = $row_lunchin['time'];
		if($lunch_in == ""){
			$lunch_in = "-";
		}
	$qry_meetingout = mysql_query("SELECT * FROM tbl_logs WHERE date = '$date' AND emp_id = '$emp_id' AND status = 'Meeting' AND action = 'Out' ORDER BY log_id ASC");
	$row_meetingout = mysql_fetch_array($qry_meetingout);
		$meeting_out = $row_meetingout['time']." - ".$row_meetingout['company'];
		if($meeting_out == ""){
			$meeting_out = "-";
		}
	$qry_meetingin = mysql_query("SELECT * FROM tbl_logs WHERE date = '$date' AND emp_id = '$emp_id' AND status = 'Meeting' AND action = 'In' ORDER BY log_id DESC");
	$row_meetingin = mysql_fetch_array($qry_meetingout);
		$meeting_in = $row_meetingin['time'];
		if($meeting_in == ""){
			$meeting_in = "-";
		}
	$qry_timeout = mysql_query("SELECT * FROM tbl_logs WHERE date = '$date' AND emp_id = '$emp_id' ORDER BY log_id DESC");
	$row_timeout = mysql_fetch_array($qry_timeout);
		$attendance_out = $row_timeout['time'];
		if($attendance_out == ""){
			$attendance_out = "-";
