...
while($row_log = mysql_fetch_array($qry_log)){
	...
	$qry_timeout = mysql_query("SELECT * FROM tbl_logs USE INDEX (emp_id,company_id,date) WHERE company_id = '$company_id' AND date = '$date' AND emp_id = '$emp_id' AND action = 'Out' ORDER BY time DESC");
...
while($row_log = mysql_fetch_array($qry_log)){
	...
	$row_fn = mysql_fetch_array($qry_fn);
		 ...
		 $sl = $row_fn['sick_leave'];
		 $vl = $row_fn['vacation_leave'];
	$qry_timein = mysql_query("SELECT * FROM tbl_logs USE INDEX (date,emp_id) WHERE date = '$date' AND emp_id = '$emp_id' ORDER BY time ASC LIMIT 1");
	$row_timein = mysql_fetch_array($qry_timein);
		$attendance_in = $row_timein['time'];
		if($attendance_in == ""){
			$attendance_in = "-";
	...
	$locate_arr = array();
	$location_qry = mysql_query("SELECT * FROM tbl_locateme USE INDEX (date,emp_id) WHERE emp_id = '$emp_id' AND date = '$date' ORDER BY locateme_id DESC LIMIT 1");
	while($row_location = mysql_fetch_array($location_qry)){
		$long = $row_location['longitude'];
		$lat = $row_location['latitude'];
		$destination = $row_location['destination'];
...
while($row_payslip = mysql_fetch_array($payslip_query)){
	...
	$payslip_arr[] = array("From DATE" => 'no data',
							 "To DATE" => 'no data'
						   );
}
