...
$emp_comp_qry = mysql_query("SELECT * FROM tbl_employee WHERE employee_id = '$employee_id'");
$row_emp_comp = mysql_fetch_array($emp_comp_qry);

$company_id = $row_emp_comp['company_id'];

$from_to_query = mysql_query("SELECT * FROM tbl_payslip WHERE company_id = '$company_id' ORDER BY payslip_id DESC");
$row_from_to = mysql_fetch_array($from_to_query);
$from = $row_from_to['from_date'];
$to = $row_from_to['to_date'];

$no_days = mysql_query("SELECT DISTINCT date FROM tbl_logs WHERE company_id = '$company_id' AND date BETWEEN '$from' and '$to'");
...
$html ='';
				...
				$qry_log = mysql_query("SELECT * FROM tbl_employee WHERE inactive != '1' AND company_id = '$company_id' AND employee_id = '".$_GET['employee_id']."' LIMIT 60");
				$xxx = 0;
...
'.$fullname.'<br>
Cut Off Period : September 01, 2015 To September 15, 2015<br/>
Number of Working days : 11<br/>
Number of days present : 11<br/>
...
<br/>
<br/>
<div style="text-align:right"><b>Net Pay : PHP '.number_format(round((($basic_rate + $overtime_ammount + $basic_allowance - $tax)-($absent_deduction + $undertime_ammount + $late_deduction + ($sss_loan + $hdmf_loan + $phic_loan + $staff_loan) + $sss + $pagibig + $philhealth)),2)).'</b></div>
